...
function obtenerUsuarios(req, res){
  //aquí estamos obteniendo los datos del usuario que está en la sesión (el que se autenticó)
...
function modificarUsuarios(req, res, next) {
...
function iniciarSesion(req, res, next){
  if (!req.body.email || !req.body.password){
    //422 FALTA INFORMACIÓN -- la info que mandó el cliente está incompleta
    return res.status(422).json({error: {emial: "Falta informacion"}}); 
  }
